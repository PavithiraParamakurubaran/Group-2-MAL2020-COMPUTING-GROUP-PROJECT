...
export default function StudentLogin() {
  ...
  // Auto-redirect if already logged in and role is student
  useEffect(() => {
    const user = localStorage.getItem("user");
    if (user) {
      const role = JSON.parse(user).role;
      if (role === "student") {
        nav("/student/dashboard"); // only redirect if role is student
      }
    ...
    <div className="login-page" style={{ backgroundImage: `url(${background})` }}>
      <div className="overlay">
        <div className="login-container">
          ...
          <div className="forgot" onClick={() => nav("/forgot-password/student")}>
            Forgot Password?
          </div>
